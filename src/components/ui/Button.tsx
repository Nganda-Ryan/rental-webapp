...
const Button = ({ children, variant = 'neutral', disable = false, onClick, isSubmitBtn = false, fullWidth = true, loading = false }: ButtonProps) => {
    ...
    const classes = `${widthClass} w-fit inline-flex items-center justify-center flex-nowrap gap-2 px-3.5 py-1.5 rounded transition whitespace-nowrap ${VARIANTS[variant] || ''} ${disable ? 'opacity-50 cursor-not-allowed' : ''}`;
